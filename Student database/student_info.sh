...
echo "$($PSQL "select major from students right join majors on students.major_id = majors.major_id where first_name ilike '%ma%' or student_id is null order by major")"
